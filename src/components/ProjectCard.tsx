...
export const ProjectCard: React.FC<ProjectCardProps> = ({ project }) => {
  ...
  return (
    <Link
      ...
      className="bg-white rounded-2xl sm:rounded-3xl border border-slate-200/90 shadow-[0_4px_20px_rgba(0,0,0,0.03)] hover:shadow-[0_20px_45px_rgba(37,99,235,0.1)] hover:border-blue-400/80 hover:-translate-y-1.5 transition-all duration-300 ease-out flex flex-col justify-between overflow-hidden group h-full block cursor-pointer"
    >
      ...
      <div className="relative aspect-[16/10] w-full overflow-hidden bg-slate-900 border-b border-slate-100">
        ...
        <div className="absolute top-3 right-3 z-10 pointer-events-none">
          <span className="px-2.5 py-1 rounded-lg bg-white/95 backdrop-blur-md text-slate-800 text-[11px] font-bold border border-slate-200/90 shadow-xs tracking-tight">
            {project.tag}
          </span>
        </div>
      ...
      <div className="p-5 sm:p-6 flex-1 flex flex-col justify-between space-y-4">
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between gap-2.5 pb-3 border-b border-slate-100">
            <div className="flex items-center gap-2.5 min-w-0 flex-1">
              <div className="w-8 h-8 rounded-xl bg-slate-50 border border-slate-200/90 p-1.5 flex items-center justify-center shrink-0 shadow-2xs group-hover:border-blue-200 group-hover:bg-blue-50/40 transition-colors">
                {getClientLogo(project.id, 'w-4 h-4')}
              </div>
              <div className="min-w-0 flex-1">
                <span className="text-xs sm:text-sm font-bold text-slate-900 truncate block tracking-tight group-hover:text-blue-600 transition-colors">
                  {project.clientName}
                </span>
              </div>
            </div>
            <span className="shrink-0 px-2.5 py-0.5 rounded-md bg-slate-100 text-slate-600 text-[10px] font-semibold border border-slate-200/60">
              {language === 'id' ? project.clientSectorId : project.clientSectorEn}
            </span>
          </div>

          {/* Project Title */}
          <h3 className="text-base sm:text-lg font-bold text-slate-900 group-hover:text-blue-600 transition-colors leading-snug line-clamp-2 tracking-tight">
          ...
          {project.metrics && project.metrics.length > 0 && (
            <div className="grid grid-cols-2 gap-2.5 p-3 rounded-xl bg-slate-50/90 border border-slate-200/70 group-hover:bg-blue-50/30 group-hover:border-blue-100 transition-all duration-200">
              <div className="min-w-0">
                <span className="text-xs sm:text-sm font-bold text-slate-900 group-hover:text-blue-600 transition-colors block truncate leading-tight">
                  {project.metrics[0].value}
                </span>
                <span className="text-[10px] sm:text-[11px] text-slate-500 font-medium truncate block mt-0.5">
        ...
        <div className="pt-3.5 border-t border-slate-100 flex items-center justify-between text-xs">
          <div className="flex items-center gap-1.5 text-slate-500">
            <Clock className="w-3.5 h-3.5 text-slate-400 group-hover:text-blue-600 transition-colors shrink-0" />
            <span className="text-xs font-medium text-slate-600">
              {project.timeline}
            </span>
          </div>

          <div className="inline-flex items-center gap-1.5 font-bold text-xs text-blue-600 group-hover:text-blue-700 transition-colors">
            <span>{language === 'id' ? 'Pelajari Kasus' : 'View Case Study'}</span>
            <div className="w-6 h-6 rounded-full bg-blue-50 group-hover:bg-blue-600 text-blue-600 group-hover:text-white flex items-center justify-center transition-all duration-200 shadow-2xs group-hover:shadow-xs">
