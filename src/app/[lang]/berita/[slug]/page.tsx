  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col relative selection:bg-blue-600 selection:text-white">
      ...
      <main className="flex-1 max-w-4xl mx-auto px-4 sm:px-8 py-6 sm:py-10 w-full">
        {/* Breadcrumb Bar */}
        <nav className="flex items-center gap-2 text-xs text-slate-400 font-medium mb-6 overflow-x-auto whitespace-nowrap pb-1">
          <Link href={`/${language}`} className="hover:text-blue-600 transition-colors">
            {t('nav-home')}
          </Link>
          <ChevronRight className="w-3.5 h-3.5 shrink-0" />
          <Link href={`/${language}/${language === 'en' ? 'news' : 'berita'}`} className="hover:text-blue-600 transition-colors">
            {t('news-header-title')}
          </Link>
          <ChevronRight className="w-3.5 h-3.5 shrink-0" />
          <span className="text-slate-600 truncate max-w-[240px] sm:max-w-md">{title}</span>
        </nav>

        {/* Top Reference Card Container (Cover + Header + Metrics) */}
        <FadeIn direction="up">
          <div className="bg-white rounded-3xl border border-slate-200/90 shadow-sm overflow-hidden mb-8 sm:mb-10">
            {/* 1. Cover Image at the Top */}
            <div className="w-full aspect-[16/9] sm:aspect-[21/10] overflow-hidden bg-slate-100">
              <img
            ...
            <div className="p-6 sm:p-8 lg:p-9">
              ...
              <div className="flex items-center justify-end gap-3 sm:gap-5 text-xs font-semibold text-slate-500 mb-4">
                <span
              ...
              <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6">
                ...
                <div className="flex-1 min-w-0">
                  <h1 className="text-2xl sm:text-3xl lg:text-[2.2rem] font-extrabold text-slate-900 tracking-tight leading-tight mb-4">
                    {title}
                  </h1>
                  ...
                  <div className="space-y-1 text-xs sm:text-sm">
                    <p className="text-slate-500 font-normal">
                      <span className="text-slate-400 font-medium mr-1.5">{t('news-by')}</span>
                      {author ? (
                        <Link
                          href={`/${language}/team/${author.id}`}
                          className="font-bold text-slate-900 uppercase hover:text-blue-600 transition-colors cursor-pointer"
                        >
                          {author.name}
                        </Link>
                      ) : (
                        <span className="font-bold text-slate-900 uppercase">Rynertia Arc Tech Editorial</span>
                      )}
                    </p>
                ...
                <div className="flex flex-row md:flex-col items-center sm:items-stretch gap-2.5 shrink-0 pt-1">
                  ...
                  >
                    <Bookmark
                      className={`w-3.5 h-3.5 ${
                        isBookmarked ? 'fill-white text-white' : 'text-slate-600'
                      }`}
                    />
                  ...
                  <button
                    ...
                    className="flex items-center justify-center gap-2 px-4 py-2 rounded-full bg-blue-50 hover:bg-blue-100 text-blue-600 border border-blue-100 text-xs font-semibold transition-all cursor-pointer shadow-2xs min-h-[40px]"
                  >
                    {copied ? (
                      <>
                        <Check className="w-3.5 h-3.5 text-emerald-600" />
                        <span className="text-emerald-700 font-bold">{t('news-copied')}</span>
                      </>
                    ) : (
                      <>
        ...
        {keyTakeaways && keyTakeaways.length > 0 && (
          <FadeIn direction="up" delay={0.15}>
            <div className="p-5 sm:p-7 rounded-2xl bg-gradient-to-br from-blue-50/90 to-sky-50/60 border border-blue-200/80 mb-10 shadow-2xs">
              <h2 className="text-xs sm:text-sm font-bold text-blue-900 uppercase tracking-wider mb-3.5 flex items-center gap-2">
        ...
        <article className="prose prose-slate max-w-none space-y-8 sm:space-y-10 text-slate-800 text-sm sm:text-base leading-relaxed">
          {article.sections.map((section, idx) => {
            ...
            return (
              <section key={idx} className="space-y-4">
                {heading && (
                  <h2 className="text-lg sm:text-2xl font-bold text-slate-900 tracking-tight mt-6 sm:mt-8 pb-1">
                    {heading}
                  </h2>
                )}
                ...
                {quote && (
                  <blockquote className="my-6 p-5 sm:p-6 rounded-2xl bg-white border-l-4 border-blue-600 border border-slate-200/80 shadow-xs italic text-slate-800 font-medium text-sm sm:text-base">
                    "{quote}"
                  </blockquote>
                )}
        ...
        {/* Author Bio Card */}
        {author && (
          <div className="mt-12 sm:mt-16 p-6 sm:p-8 rounded-3xl bg-white border border-slate-200/80 shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl overflow-hidden bg-slate-100 ring-2 ring-blue-100 shrink-0">
                <img
                  src={author.image}
                  alt={author.name}
                  className="w-full h-full object-cover"
              ...
              <div>
                <span className="text-xs text-blue-600 font-bold uppercase tracking-wider block mb-0.5">
                ...
                <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
                  {authorRole} &bull; <span className="text-blue-600 font-medium">{authorDept}</span>
                </p>
              </div>
            </div>
            ...
            <Link
              href={`/${language}/team/${author.id}`}
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-slate-900 hover:bg-blue-600 text-white text-xs font-bold transition-all shrink-0 group/btn"
            >
              ...
              {relatedArticles.map(related => {
                ...
                return (
                  <Link
                    ...
                    className="p-4 rounded-2xl bg-white border border-slate-200/80 shadow-xs hover:shadow-md hover:border-blue-300 transition-all flex flex-col group"
                  >
